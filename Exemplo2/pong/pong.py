# By Tristan Fogt
# Disponível em < https://github.com/Its-Triggy/PongEvolution/blob/master/pongEvolution.py />
import pygame
import numpy as np
import random
BLACK = (0,0,0)
...
def displayNetwork(layer_sctructure, coefs = testCoefs, command = 0):
	...
	inputs = ["pos", "bola x", "bola y", "bv_x", "bc_y"]
	outputs = ["esq", "dir", "para"]
...
while not done:
	...
	for i, paddle in enumerate(paddles):
		...
		input = np.array( [ [paddle.x, balls[i].x, balls[i].y, balls[i].xspeed , balls[i].yspeed] ])
	...
	text = font.render("Pontos = " + str(high_score), True, TEXT)
	screen.blit(text,[size[0]-300,30])	
	text2 = font.render("Vivos = " + str(still_alive), True, TEXT)
	screen.blit(text2,[size[0]-300, 90])	
	text2 = font.render("Gen = " + str(generation), True, TEXT)
